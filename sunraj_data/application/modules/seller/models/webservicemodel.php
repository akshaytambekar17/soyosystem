...
function register_to_user($form_data)
	{
		$this->db->insert('veg_users',$form_data);	
...
//start check otp created by sumit
 function login_data($data,$registration_id)
		 {	//print_r($data);exit;
			...
			$this->db->update('veg_users', $registration_id);
			$this->db->select('*');
			$this->db->from('veg_users');
			$this->db->where('password' , $data['password']);
			$this->db->where('email_id' , $data['email_id']);
	...
	{
		$this->db->insert('veg_category',$form_data);	
		if($this->db->affected_rows() == 1)
		{
			return true;
		}
	...
	{
		$this->db->insert('veg_subscriber',$form_subs);	
		if($this->db->affected_rows() == 1)
		{
			return true;
		}
...
function update_forgot_password($form_data)
   {
	$this->db->where('mob_no', $form_data['mob_no']);
	$this->db->update('veg_users', $form_data);		
	if ($this->db->affected_rows() == 1)
...
//start seller_veg_list by  created by sumit   03/02/16
	function seller_veg_list($user_id)
    {
        $this->db->select('*');
        $this->db->from('veg_category');
 ...
 //end seller_veg_list by  created by sumit   03/02/16
...
function subscriber_update_to_seller($form_data,$user_id)
	{
		...
		$this->db->where('user_id', $user_id);
		$this->db->update('veg_subscriber', $subscribedata);		
		$this->db->where('user_id', $user_id);
		$this->db->update('veg_subscriber', $delete);		
		
		$this->db->insert('veg_subscriber',$form_data);	
		$this->db->select('*');
        $this->db->from('veg_subscriber');
		$this->db->where('user_id', $user_id);
        $this->db->where('is_deleted', 0);
        $query = $this->db->get();
        if($query->num_rows() >= 0) 
...
//Start user_delete_to_seller created by sumit  03/02/16
	function user_delete_to_seller($user_id,$delete)
    {
        $this->db->where('user_id', $user_id);
       // $this->db->where('unique_id', $unique_id);
        $this->db->update('veg_users', $delete);
		//print_r($this->db->last_query());exit;				
		if ($this->db->affected_rows() == 1)
		{
			return true;
...
//start add_veg_category created by sumit  03/02/16
function add_veg_category($form_data) 
    {
	$this->db->insert('veg_category',$form_data);
...
function update_price_to_category($user_id,$category_id,$form_data)
    {
        ...
        $this->db->update('veg_category', $form_data);
		if ($this->db->affected_rows() == 1)
		{
			return true;
		}
...
function selectselleritem($form_data) 
    {
	$this->db->insert('veg_checkseller',$form_data);
...
function view_todays_item_user($userid,$ads_sub_category_id)
    {  
        $this->db->select('*');
        $this->db->from('veg_category');
...
function view_orderlist_item_seller($login_id)
    {  
        ...
        $this->db->from('veg_orders AS a');
        $this->db->join('veg_users AS b', 'b.user_id=a.user_id','INNER');		
...
function booking_all_amount($form_data) 
    {
	$this->db->insert('veg_orders',$form_data);
...
function booking_to_item($data) 
    {
	$this->db->insert('veg_order_items',$data);
           $this->db->select('*');
        $this->db->from('veg_order_items');
...
function details_of_user_place_order_old($category_id)
    {
		$query = $this->db->query("select * from veg_order_items where category_id  IN($category_id) ");
...
function details_of_user_place_order($order_id)
    { 
		$this->db->select('a.*,b.*');
		$this->db->from('veg_order_items AS a');
		$this->db->join('veg_category AS b', 'b.category_id = a.category_id', 'INNER');
...
function user_cancel_book_item($category_id,$order_id )
    {
        ...
        $this->db->delete('veg_order_items');
		if ($this->db->affected_rows() == 1)
		{
			return true;
		}
...
function update_quantity($form_data,$order_id,$category_id )
   {
		$this->db->where('order_id', $order_id);
		$this->db->where('category_id', $category_id);
		$this->db->update('veg_order_items', 	$form_data);	
		if ($this->db->affected_rows() >= 1)
		{
			return TRUE;
		}
   ...
   {		
		$this->db->where('order_id', $order_id);
		$this->db->where('category_id', $category_id);
		$this->db->update('veg_order_items', 	$se);	
   ...
   {		
		$this->db->where('order_id', $order_id);
		$this->db->update('veg_orders', 	$Amount);	
...
//end//
 ...
 function update_order_status($data)
   {		
		$this->db->where('order_id',$data['order_id']);
		$this->db->update('veg_orders',$data);	
   ...
   function report($data)
    {
		...
		$a=$data['from_date'];
		$b=$data['to_date'];
		$query= $this->db->query("SELECT * FROM `veg_orders` WHERE is_deleted !=0 AND  (order_date BETWEEN '$a' AND '$b')");
   ...
   function offer_list($login_id)
	...
	{		
	 ...
	 function update_veg_category($form_data)
   {		
   ...
   function select_category()
	...
	{
		$this->db->select('*');
		$this->db->from('veg_ads_sub_category');
	...
	function new_add_veg_category($form_data)
    {
		$this->db->insert('veg_category',$form_data);	
		if ($this->db->affected_rows() == 1)
		{
   ...
   function update_stock($data)
   {
	   $this->db->where('category_id',$data['category_id']);
		$this->db->update('veg_category',$data);	
 ...
 function add_main_category($form_data)
	{
		$this->db->insert('veg_ads_category',$form_data);	
		if($this->db->affected_rows() == 1)
		{
			return true;
		}
 ...
 function add_sub_category($form_data)
	{
		$this->db->insert('veg_ads_sub_category,',$form_data);	
		if($this->db->affected_rows() == 1)
		{
			return true;
		}
