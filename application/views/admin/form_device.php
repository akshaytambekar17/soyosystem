	...
	$(document).ready(function(){
        ...
        $('#add_device_paramters').on('click',function(e){
         	...
         	if(count <= max_fields_cms){
	            ...
	            html+='<div class="col-sm-8 private-public">';
	            html+='<div class="form-group field-memberfamilyrelation-name">';
	            html+='<input aria-invalid="true" id="device_parameter" class="form-control" name="device_parameter['+count+']" maxlength="100" type="text">';
	            html+='</div></div>';
             	html+='<div class="col-sm-4 text-right pull-right"><a href="javascript:void(0)" class="remove_field remove-family-member btn btn-danger waves-effect waves-light">×</a></div>';
                html+='</div>';
                $(wrapper).append(html);  
                count++;
        	}else{
